...
export function Visualizations() {
  ...
  const [activeVizTab, setActiveVizTab] = useState('heatmap'); // 'heatmap' | 'vectors' | 'histogram'
  const [colorMap, setColorMap] = useState('polar'); // 'polar' | 'thermal' | 'grayscale'
  ...
  // Render 316x332 Matrix on HTML5 Canvas with Colormap
  useEffect(() => {
    ...
    // Simulate 316x332 raster matrix
    for (let r = 0; r < height; r++) {
      for (let c = 0; c < width; c++) {
        const idx = (r * width + c) * 4;
        
        // Centered distance from South Pole (approx row 158, col 166)
        const dr = (r - 158) / 158;
        const dc = (c - 166) / 166;
        const dist = Math.sqrt(dr * dr + dc * dc);

        // Continental land mask vs sea ice ring
        let val = 0;
        if (dist < 0.45) {
          // Antarctic land ice sheet (high fixed value)
          val = 0.95;
        } else if (dist < 0.85) {
          // Pack ice zone with longitudinal variance and forecastDay shifts
        ...
        if (val === 0) {
          data[idx] = isDark ? 2 : 240;     // R
          data[idx + 1] = isDark ? 6 : 249; // G
          data[idx + 2] = isDark ? 23 : 255;// B
          data[idx + 3] = 255;              // Alpha
        } else if (colorMap === 'polar') {
          // Dark Navy -> Cyan -> White
  ...
  // ERA5 Synthetic Wind & Ocean Vectors Sample Set
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-border/60 light:border-slate-light-border pb-5">
        ...
        {/* View Switcher Tabs */}
        <div className="flex items-center p-1 rounded-xl bg-ocean-navy light:bg-slate-100 border border-slate-border light:border-slate-light-border">
          <button
            onClick={() => setActiveVizTab('heatmap')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              activeVizTab === 'heatmap'
                ? 'bg-ice-cyan text-midnight shadow-glow-cyan'
      ...
      {activeVizTab === 'heatmap' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 p-6 rounded-2xl border border-slate-border light:border-slate-light-border bg-ocean-navy/70 light:bg-white space-y-4 shadow-lg flex flex-col items-center">
            ...
            {/* Canvas Container */}
            <div className="relative border border-slate-border rounded-xl overflow-hidden bg-midnight shadow-inner max-w-md w-full aspect-[332/316]">
              <canvas
                ref={canvasRef}
            ...
            {/* Coordinate Inspector Probe */}
            <div className="w-full p-3 rounded-xl border border-slate-border/80 bg-midnight/50 light:bg-slate-50 flex items-center justify-between text-xs font-mono">
              {hoverGridCell ? (
                <>
                  <div className="flex items-center gap-2">
          ...
          {/* Right Col: Color Maps & Filter Sliders */}
          <div className="p-6 rounded-2xl border border-slate-border light:border-slate-light-border bg-ocean-navy/70 light:bg-white space-y-6 shadow-lg">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`py-2 px-1 text-center rounded-lg text-xs font-mono border transition-all ${
                      colorMap === p.id
                        ? 'bg-ice-cyan text-midnight font-bold border-ice-cyan shadow-glow-cyan/40'
                        : 'bg-midnight/60 light:bg-slate-50 text-slate-300 light:text-slate-700 border-slate-border hover:border-ice-cyan/50'
                    }`}
                  >
                    {p.label}
                  </button>
            ...
            {/* Minimum Concentration Threshold Slider */}
            <div className="space-y-2">
            ...
            {/* Mathematical Metadata Card */}
            <div className="p-4 rounded-xl border border-slate-border/70 bg-midnight/50 light:bg-slate-50 space-y-2 text-xs font-mono">
      ...
      {activeVizTab === 'vectors' && (
        <div className="p-6 rounded-2xl border border-slate-border light:border-slate-light-border bg-ocean-navy/70 light:bg-white space-y-6 shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-border/60 pb-3">
            <div>
              <h3 className="font-bold text-sm text-white light:text-ocean-navy">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {vectorPoints.map(v => (
              <div
                key={v.id}
                className="p-4 rounded-xl border border-slate-border light:border-slate-light-border bg-midnight/50 light:bg-slate-50 space-y-2 text-xs font-mono"
              >
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-border/50 text-[11px]">
                  <div>
      ...
      {activeVizTab === 'histogram' && (
        <div className="p-6 rounded-2xl border border-slate-border light:border-slate-light-border bg-ocean-navy/70 light:bg-white space-y-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            <div className="p-3 rounded-xl bg-midnight/50 light:bg-slate-50 border border-slate-border text-xs font-mono">
